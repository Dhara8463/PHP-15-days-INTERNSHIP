...
if(!isset($_GET['eid']) || empty($_GET['eid'])){
    header("location:displayproject.php");
}

$selectq = mysqli_query($connection, "select * from tbl_projec where pro_id='{$editid}' " ) or die(mysqli_error($connection));
...
if($_POST){
 ...
 $query = mysqli_query($connection,"update  tbl_projec set  pro_name='{$pname}',pro_price='{$pprice}',pro_details='{$pdetails}' where pro_id='{$editid}' ")
            or die(mysqli_error($connection));
    if($query){
      
        echo "<script>alert('Record updated');window.location='displayproject.php';</script>";
    }
                ...
                <div class="inner_content_w3_agile_info two_in">
                    ...
                    <div class="forms-main_agileits">
                        ...
                        <div class="wthree_general graph-form agile_info_shadow ">
                            <h3 class="w3_inner_tittle two">Project details Edit</h3></a>

                            <div class="grid-1 ">
                                <div class="form-body">
                                    <form class="form-horizontal" method="post" enctype="multipart/form-data">
                                        <input type="hidden" name="id" value="<?php echo $_selectrow['pro_id'] ?>" />
                                        <div class="form-group">
                                            <label for="focusedinput" class="col-sm-2 control-label">Project Name</label>
                                            <div class="col-sm-8">
                                                <input type="text" class="form-control1" id="focusedinput" value="<?php echo $selectrow['pro_name'] ?>" name="pname" placeholder="Enter Your Project Name " required>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="focusedinput" class="col-sm-2 control-label">Project Price</label>
                                            <div class="col-sm-8">
                                                <input type="text" class="form-control1" id="focusedinput" value="<?php echo $selectrow['pro_price'] ?>" name="pprice" placeholder="Enter Your Project price" required>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="focusedinput" class="col-sm-2 control-label">Project Details</label>
                                            <div class="col-sm-8">
                                                <textarea  class="form-control1"name="pdetails" required><?php echo $selectrow['pro_details'] ?></textarea>
                                            </div>
                                        </div>
